...
3. `003_internal_test_writes.sql`
   - `leave_requests` へのINSERT候補です。
   - 有給申請登録のDual Write検証限定です。
   - `UPDATE` / `DELETE` は許可しません。
...
- `USE_SUPABASE_WRITE=true` は、有給申請登録だけをSpreadsheet保存成功後にSupabaseへDual Writeする検証用です。
  Supabase INSERTに失敗してもSpreadsheetはロールバックしません。
...
- 内部書き込み検証限定: `leave_requests` INSERT
